...
import { PredictWebSocketPool } from './websocket/pool.js';
import { handleMessage, shutdownWriters } from './websocket/handlers.js';

const logger = createLogger('predict-listener');

let wsPool: PredictWebSocketPool | null = null;
let marketRefreshInterval: ReturnType<typeof setInterval> | null = null;
let cleanupInterval: ReturnType<typeof setInterval> | null = null;

async function main(): Promise<void> {
  const config = loadConfig();
  logger.info({ restUrl: config.restUrl, wsUrl: config.wsUrl }, 'Starting Predict.fun listener');
  ...
  // Initial market sync
  const { marketIds } = await refreshMarkets(config);

  // Initialize WebSocket pool
  wsPool = new PredictWebSocketPool(
    config.wsUrl,
    config.apiKey,
  ...
  await wsPool.subscribeToMarkets(marketIds);
  logger.info({
  ...
  marketRefreshInterval = setInterval(async () => {
    try {
      ...
      await wsPool?.subscribeToMarkets(updated.marketIds);
    } catch (err) {
  ...
  logger.info('Predict.fun listener started successfully');
}
